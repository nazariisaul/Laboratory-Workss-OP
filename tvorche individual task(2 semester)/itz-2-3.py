...
class PhysicsLab(tk.Tk):
    ...
    def _build_vyshyvanka_strip(self):
        """
        Малює декоративну смугу вишиванки між заголовком і вкладками.
        Відображається ТІЛЬКИ 21 травня (День вишиванки).

        Режими (за пріоритетом):
          1. Якщо поряд із скриптом є файл vyshyvanka_tile.png — тайл-режим:
             зображення повторюється горизонтально, як тканина.
          2. Якщо файл не знайдено — піксельний орнамент (вбудований fallback).
        """
        ...
        C_RED = "#c8102e"
        C_BLACK = "#1a1a1a"
        C_GOLD = "#d4a017"  # золота рамка — нова деталь
        C_WHITE = "#f5f0e8"
        STRIP_H = 56  # трохи вища смуга для кращого вигляду

        # ── Canvas ───────────────────────────────────────────────────────
        canvas = tk.Canvas(self, height=STRIP_H, bg=C_WHITE,
        ...
        def _show_tip(event):
            ...
            tip = tk.Label(self,
                           ...
                           bg=C_RED, fg=C_WHITE, pady=3, padx=8,
                           relief="flat")
            tip.place(relx=1.0, rely=0, anchor="ne", x=-14, y=STRIP_H + 4)
            self.after(3500, lambda: (tip.destroy(),
                                      _tip_shown.__setitem__(0, False)))
        ...
        _img_candidates = [
            _script_dir / "vyshyvanka_tile.png",
            _script_dir / "assets" / "vyshyvanka_tile.png",
            pathlib.Path("vyshyvanka_tile.png"),
        ]
        _img_path = next((p for p in _img_candidates if p.exists()), None)

        # Зберігаємо PhotoImage тут, щоб не зібрав GC
        _photo_ref = [None]

        # ── Режим 1: Зображення-тайл ─────────────────────────────────────
        if _img_path:
            try:
                from PIL import Image as _PILImage, ImageTk as _ImageTk

                _pil_img = _PILImage.open(_img_path).convert("RGBA")
                # Масштабуємо висоту під зону орнаменту (без рамок)
                _zone_h = STRIP_H - 12
                _ratio = _zone_h / _pil_img.height
                _tile_w = max(1, int(_pil_img.width * _ratio))
                _pil_tile = _pil_img.resize((_tile_w, _zone_h),
                                            _PILImage.LANCZOS)

                def _draw_image(event=None):
                    canvas.delete("all")
                    ...
                    # Тайлінг зображення
                    photo = _ImageTk.PhotoImage(_pil_tile)
                    _photo_ref[0] = photo  # тримаємо посилання!
                    x = 0
                    while x < w:
                        canvas.create_image(x, 6, anchor="nw", image=photo)
                        x += _tile_w

                    # Рамки поверх зображення
                    _draw_borders(canvas, w, STRIP_H, C_BLACK, C_RED, C_GOLD)

                canvas.bind("<Configure>", _draw_image)
                self.after(50, _draw_image)
                return  # ← успішно, виходимо

            except ImportError:
                pass  # Pillow не встановлено — падаємо на fallback
            except Exception:
                pass  # Будь-яка помилка — теж fallback

        # ── Режим 2: Вбудований піксельний орнамент (fallback) ───────────
        MOTIF = [
            "R...R.BBBBB.R...R.BBBBB.R...R.BBBBB.R...R.BBBBB.R...R.BBBBB",
            "RR.RR.B.R.B.RR.RR.B.R.B.RR.RR.B.R.B.RR.RR.B.R.B.RR.RR.B.R.B",
            "RRRRR.BRBBB.RRRRR.BRBBB.RRRRR.BRBBB.RRRRR.BRBBB.RRRRR.BRBBB",
            "RR.RR.BBBBB.RR.RR.BBBBB.RR.RR.BBBBB.RR.RR.BBBBB.RR.RR.BBBBB",
            "R...R.B.R.B.R...R.B.R.B.R...R.B.R.B.R...R.B.R.B.R...R.B.R.B",
        ...
        def _draw_pixel(event=None):
            canvas.delete("all")
            w = canvas.winfo_width() or self.winfo_width()

            canvas.create_rectangle(0, 0, w, STRIP_H, fill=C_WHITE, outline="")
            ...
            color_map = {"R": C_RED, "B": C_BLACK, "G": C_GOLD}

            for rep in range(repeats):
                ox = rep * tile_w
                for ri, row in enumerate(MOTIF):
                    y0, y1 = zone_y + ri * cell, zone_y + ri * cell + cell
                    for ci, ch in enumerate(row):
                        ...
                        canvas.create_rectangle(
                            x0, y0, x0 + cell, y1,
                            fill=color_map.get(ch, C_RED), outline=""
                        )

            # Зірки між мотивами
            star_r = cell * 2.0
            for rep in range(repeats):
                cx = rep * tile_w + tile_w // 2
                cy = zone_y + zone_h // 2
                for angle_deg in range(0, 360, 45):
                    import math
                    a = math.radians(angle_deg)
                    canvas.create_line(
                        int(cx + math.cos(a) * cell * 0.4),
                        int(cy + math.sin(a) * cell * 0.4),
                        int(cx + math.cos(a) * star_r),
                        int(cy + math.sin(a) * star_r),
                        fill=C_GOLD, width=max(1, cell // 3)
                    )
                canvas.create_oval(
                    cx - cell // 2, cy - cell // 2,
                    cx + cell // 2, cy + cell // 2,
                    fill=C_BLACK, outline=C_GOLD, width=1
                )

            _draw_borders(canvas, w, STRIP_H, C_BLACK, C_RED, C_GOLD)

        canvas.bind("<Configure>", _draw_pixel)
        self.after(50, _draw_pixel)

    def _draw_borders(canvas, w, h, black, red, gold):
        """Малює декоративні рамкові смуги зверху і знизу canvas."""
        # Зверху: чорна → червона → золота
        canvas.create_rectangle(0, 0, w, 2, fill=black, outline="")
        canvas.create_rectangle(0, 2, w, 5, fill=red, outline="")
        canvas.create_rectangle(0, 5, w, 7, fill=gold, outline="")
        # Знизу: золота → червона → чорна
        canvas.create_rectangle(0, h - 7, w, h - 5, fill=gold, outline="")
        canvas.create_rectangle(0, h - 5, w, h - 2, fill=red, outline="")
        canvas.create_rectangle(0, h - 2, w, h, fill=black, outline="")
